...
pygame.display.set_caption("PD + Interactive Animations + Pause")
...
# Arc & gauge scales, gauge rotation, handle angle
...
# --------------------------------------------------------------------------------
# 3) Double-Click to Pause Logic
# --------------------------------------------------------------------------------
paused = False
last_click_time = 0
DOUBLE_CLICK_TIME = 300  # ms

font = pygame.font.SysFont(None, 64)  # For the "PAUSE" text
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.button == 1:  # Left click
                ...
                # If not paused, handle the normal dragging logic
                if not paused:
                    dragging = True
                    mx, my = event.pos
    ...
    if paused:
        #  -- PAUSED State --
        # Show lines/dots from PD if you want them to remain
        # visible, or comment out to hide them entirely. We'll show them for demonstration:
        ...
        # You can skip drawing arcs/gauge/handle to reduce CPU further
        # or keep them visible if desired (frozen).
        # Let's skip them for minimal CPU usage:
        # (No animations, no re-draw of arcs/gauge/handle)
        pass

    else:
        #  -- ACTIVE State --
        # Update arc/gauge scales
        ...
        # 2) Draw mask, boldcircle, thincircle (no scaling)
        screen.blit(mask_surf, mask_rect)
        screen.blit(boldcircle_surf, boldcircle_rect)
        screen.blit(thincircle_surf, thincircle_rect)

        # 3) Draw gauge (scale + rotate)
        gw, gh = gauge_surf.get_size()
        gauge_scaled_w = int(gw * gauge_scale_current)
        gauge_scaled_h = int(gh * gauge_scale_current)
        gauge_scaled   = pygame.transform.scale(gauge_surf, (gauge_scaled_w, gauge_scaled_h))
        ...
        thickarc_w = int(tw * arc_scale_current)
        thickarc_h = int(th * arc_scale_current)
        thickarc_scaled = pygame.transform.scale(thickarc_surf, (thickarc_w, thickarc_h))
        thickarc_scaled_rect = thickarc_scaled.get_rect(center=(WIDTH//2, HEIGHT//2))
        screen.blit(thickarc_scaled, thickarc_scaled_rect)

        tw2, th2 = thinarc_surf.get_size()
        thinarc_w = int(tw2 * arc_scale_current)
        thinarc_h = int(th2 * arc_scale_current)
        thinarc_scaled = pygame.transform.scale(thinarc_surf, (thinarc_w, thinarc_h))
        thinarc_scaled_rect = thinarc_scaled.get_rect(center=(WIDTH//2, HEIGHT//2))
        screen.blit(thinarc_scaled, thinarc_scaled_rect)

        # 5) Draw handle (rotation only)
